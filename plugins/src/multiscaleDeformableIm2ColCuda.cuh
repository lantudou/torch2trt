    ...
    int32_t const numQuery, int32_t const numPoint, scalar_t* dataCol)
{
    ...
    if (err != cudaSuccess)
    {
        std::cerr  << "error in ms_deformable_im2col_cuda: " << cudaGetErrorString(err) << std::endl;
    }
}
    ...
    scalar_t* gradAttnWeight)
{
    ...
    if (err != cudaSuccess)
    {
       std::cerr  <<  "error in ms_deformable_col2im_cuda: " << cudaGetErrorString(err) << std::endl;
    }
}
